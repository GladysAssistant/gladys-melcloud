...
test('buildAirToAirFeatures exposes power, mode and target temperature', () => {
  ...
  assert.deepEqual(
    ...
    {
      external_id: 'ext:melcloud:air-to-air:123:power',
  ...
  assert.deepEqual(
    {
      external_id: temperature.external_id,
      category: temperature.category,
      type: temperature.type,
    },
    {
      external_id: 'ext:melcloud:air-to-air:123:temperature',
      category: 'air-conditioning',
...
test('buildSetAtaChanges maps Gladys commands to SetAta payloads', () => {
  ...
  assert.deepEqual(buildSetAtaChanges('mode', AC_MODE.COOLING), {
    EffectiveFlags: 6,
    OperationMode: 3,
  });
  assert.deepEqual(buildSetAtaChanges('mode', AC_MODE.AUTO), {
    EffectiveFlags: 6,
    OperationMode: 8,
  });
  assert.deepEqual(buildSetAtaChanges('temperature', 22.5), {
    EffectiveFlags: 4,
    SetTemperature: 22.5,
  });
...
test('mode mappings round-trip between Gladys and MELCloud', () => {
  ...
  Object.entries(melCloudModes).forEach(([melCloudMode, gladysMode]) => {
    const [state] = buildPollStates(EXTERNAL_ID, { OperationMode: Number(melCloudMode) }).filter(
      (s) => s.device_feature_external_id.endsWith(':mode'),
    );
